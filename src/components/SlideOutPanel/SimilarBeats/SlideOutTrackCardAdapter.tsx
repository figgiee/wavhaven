...
import type { SimilarTrackCardData } from '@/server-actions/tracks/trackQueries';
// Beat type imported from global types
import type { Beat } from '@/types'; // Import Beat from global types
import type { License } from '@/types'; // Import License type for clarity

interface SlideOutTrackCardAdapterProps {
  // Accept the data structure returned by the server action
  beat: SimilarTrackCardData;
  className?: string;
}

export const SlideOutTrackCardAdapter: React.FC<SlideOutTrackCardAdapterProps> = ({ beat, className }) => {
  // Adapt the SimilarTrackCardData structure to the structure expected by TrackCard
